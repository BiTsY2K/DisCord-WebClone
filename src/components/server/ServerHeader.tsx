    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <header className="relative h-12 flex items-center justify-between gap-1 font-semibold shadow-md text-[#313338] dark:text-[#dbdee1] hover:bg-[#dfe0e3] dark:hover:bg-[#383a40] px-4 py-3 transition-colors duration-100 ease-linear">
          <h2 className="text-ellipsis min-w-0 whitespace-nowrap overflow-hidden">
            {server.name}
          </h2>
      ...
      <DropdownMenuContent className="w-[13.75rem] font-medium px-2 py-1.5 hove:bg-[#949cf7]">
        <DropdownMenuItem
          onClick={() => OpenModal(Modal.ModalType.INVITE, { server })}
        >
        ...
        {isAdmin && (
          <DropdownMenuItem
            onClick={() =>
              OpenModal(Modal.ModalType.SERVER_SETTINGS, { server })
            }
          >
        ...
        {(isAdmin || isModerator) && (
          <React.Fragment>
            <DropdownMenuItem
              onClick={() =>
                OpenModal(Modal.ModalType.CREATE_CHANNEL, { server })
              }
            >
        ...
        >
          <label htmlFor="terms" className="flex flex-1">
            Hide Muted Channel
          </label>
        ...
        {!isAdmin && (
          <DropdownMenuItem
            className="text-[#da373c] focus:bg-destructive focus:text-destructive-foreground"
            onClick={() => OpenModal(Modal.ModalType.LEAVE_SERVER, { server })}
          >
            <div className="flex flex-1">Leave Server</div>
