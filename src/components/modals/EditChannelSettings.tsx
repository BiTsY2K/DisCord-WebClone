...
const settings = [{ label: "Overview", content: "" }];
...
export const ChannelSettings = () => {
  ...
  return (
    <Dialog
      open={isOpen && type === ModalType.CHANNEL_SETTINGS}
                ...
                <div className="h-full w-[calc(192px+20px+6px)] py-[60px] pr-1.5 pl-5">
                  ...
                  >
                    <div className="text-xs text-[#5c5e66] dark:text-[#878986] leading-[12px] tracking-[0.02em] font-bold uppercase text-ellipsis whitespace-nowrap overflow-hidden flex-shrink-0">
                      <Hash className="h-3 w-3 mr-1 mb-[3px] inline-block" />
                      {channel?.name}
                      <span className="font-semibold ml-1 text-[#4e5058] dark:text-[#b5bac1]">
                        {channel?.type} Channel
                      </span>
                    </div>
                  </div>

                  {/* Channel Settings Tabs Trigger */}
                  <React.Fragment>
                    {settings.map((setting, _) => (
                      <TabsTrigger
                        key={_}
                        tabIndex={0}
                        value={setting.label.toLowerCase()}
                  ...
                  <Separator className="my-2 bg-[#d7d8dd] dark:bg-[#3c3e44]" />
                  ...
                  <Button
                    ...
                    style={tabTriggerStyle}
                    onClick={() =>
                      OpenModal(ModalType.DELETE_CHANNEL, { server, channel })
                    }
                  >
                    <div className="flex flex-1">Delete Channel</div>
